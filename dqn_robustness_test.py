...
def simulate_dqn_robustness(model_path, num_episodes=20, max_steps_per_episode=2000, traffic_pattern="extreme_fluctuation_1"):
    """
    使用训练好的DQN模型进行鲁棒性测试
    测试在极端波动流量模式下的性能
    ...
    """
    print(f"开始DQN鲁棒性测试 - 流量模式: {traffic_pattern}")
    ...
    for episode in range(num_episodes):
        ...
        # 记录episode结束时的统计信息
        # 计算总生成包数（从priority_generated列表求和）
        ...
        for priority in [1, 2, 3]:
            # 计算平均延迟（从priority_transmission_delays字典获取）
            if priority in env.priority_transmission_delays and len(env.priority_transmission_delays[priority]) > 0:
                delays = env.priority_transmission_delays[priority]
                episode_stats['priority_avg_delay'][priority-1] = np.mean(delays)
            else:
    ...
    stats_dir = f"dqn_robustness_test_{timestamp}"
...
def plot_robustness_results(episode_df, step_df, save_dir):
    ...
    # 保存图表
    save_path = os.path.join(save_dir, "dqn_robustness_test_results.png")
...
if __name__ == "__main__":
    ...
    # 查找训练好的模型路径
    # 请根据实际情况修改模型路径
    #model_path = "dynamic_uav_models/dynamic_20251203_192603/dqn_dynamic_uav_final.zip"
    model_path = "/home/qwh/nndqn/dynamic_uav_models/dynamic_20260105_110637/dqn_dynamic_uav_final.zip"
