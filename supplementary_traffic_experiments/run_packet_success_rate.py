# -*- coding: utf-8 -*-
"""
补实验入口：运行三次独立流量实验的数据包成功率测试，并自动绘图。

运行（项目根目录，dqn 环境）：
    /home/qwh/miniconda3/envs/dqn/bin/python supplementary_traffic_experiments/run_packet_success_rate.py

或：
    bash supplementary_traffic_experiments/run_packet_success_rate.sh

流程：
  1. 对实验1/2/3 分别运行 DQN、SAC、CODEL
  2. 保存 CSV 到 supplementary_traffic_experiments/results/
  3. 绘制成功率柱状图到 supplementary_traffic_experiments/figures/packet_success_rate/
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="补实验：数据包成功率测试")
    parser.add_argument("--episodes", type=int, default=20, help="每个算法 episode 数")
    parser.add_argument("--max-steps", type=int, default=2000, help="每 episode 最大步数")
    parser.add_argument(
        "--dqn-model",
        type=str,
        default=None,
        help="DQN model path (153-dim obs, dqn_dynamic_uav)",
    )
    parser.add_argument("--sac-model", type=str, default=None, help="SAC 模型路径")
